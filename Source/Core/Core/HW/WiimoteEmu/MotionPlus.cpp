...
MotionPlus::ActivationStatus MotionPlus::GetActivationStatus() const
{
  ...
  {
    if (u8(ChallengeState::Activating) == m_reg_data.challenge_state)
...
int MotionPlus::BusWrite(u8 slave_addr, u8 addr, int count, const u8* data_in)
{
  ...
  case ActivationStatus::Active:
  {
    ...
    case offsetof(Register, challenge_type):
      if (u8(ChallengeState::ParameterXReady) == m_reg_data.challenge_state)
      {
        ...
        m_reg_data.challenge_state = u8(ChallengeState::PreparingY);
      }
      break;
...
void MotionPlus::Activate()
{
  ...
  m_reg_data.challenge_state = u8(ChallengeState::Activating);
...
void MotionPlus::Update()
{
  ...
  if (!m_progress_timer && ActivationStatus::Activating == GetActivationStatus())
  {
    ...
    m_reg_data.challenge_state = u8(ChallengeState::PreparingX);
  ...
  switch (ChallengeState(m_reg_data.challenge_state))
  {
  case ChallengeState::PreparingX:
    ...
    m_reg_data.challenge_state = u8(ChallengeState::ParameterXReady);
    break;
  ...
  case ChallengeState::PreparingY:
    ...
    m_reg_data.challenge_state = u8(ChallengeState::ParameterYReady);
    break;
